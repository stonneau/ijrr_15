...
%DIF ADD out_new.tex   Mon Oct  9 13:17:22 2017
%% bare_jrnl.tex
%% V1.4b
%% 2015/08/26
%% by Michael Shell
...
\DIFdelbegin \DIFdel{The paper extends our previous papers presenting the reachability condition~\cite{tonneauisrr15}and the connection of our contact planner with a solver~$\mathcal{P}_3$~\cite{Carpentier2016}.
In this paper , we introduce a new efficient criterion of the robot balance, provide the complete }\DIFdelend \DIFaddbegin \noindent \DIFadd{\textbf{Comparison with our previous work:}
...
previous work. Section~\ref{rbprm} and~\ref{sec:contact} of the present paper are thus paraphrasing the original paper, with the exception of Section~\ref{app:contact}, where we additionally provide the }\DIFaddend pseudo-code \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{of the algorithm absent from the original paper. We also improve the computational performance of the method by replacing the static equilibrium test with a more efficient test based on a new LP formulation presented in Section~\ref{sec:Equil}.
Compared with the original paper, we also }\DIFaddend release our source code\DIFdelbegin \DIFdel{that can be used to directly reproduce our results. We also provide
}\DIFdelend \DIFaddbegin \DIFadd{, and provide the complete procedure to use our planner for any legged robot. We thus provide
all the technical information required to use the planner.
}

\DIFadd{To validate our approach, in this paper we had to implement or integrate other tools based on previous publications from our team. We implemented }\DIFaddend a solution to $\mathcal{P}_3$ \DIFaddbegin \DIFadd{based on a reformulation of our previous work~\cite{Carpentier2016}. The implementation of this solution is also provided as an open-source project, which can be integrated directly in our framework. Our solution to $\mathcal{P}_3$ then allows us to verify that the synthesized motions are physically consistent, using our implementation of a state-of-the-art simulation algorithm~\cite{Kaufman2008}.
...
\DIFadd{Indeed, thanks to these tools we have been able to design a complete pipeline for synthesizing multi-contact motions from scratch.
...
thus making the planning problem even harder. Through an extensive empirical analysis, we demonstrate that the method remains interesting in this context, qualitatively and efficiently}\DIFaddend .
\DIFdelbegin \DIFdel{The planner is validated on robot models of HyQ and HRP-2 (while \cite{tonneauisrr15} used unrealistic avatar models) with complete performance analysis.
}\DIFdelend 
...
\begin{figure}
  \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\linewidth]{figures/HyQ_roms}
%DIFDELCMD <   %%%
...
\subsubsection{HRP-2 -- \DIFdelbegin \DIFdel{Steep staircase }\DIFdelend \DIFaddbegin \DIFadd{Standing up }\DIFaddend (Figure~\DIFdelbegin \DIFdel{\ref{fig:stair_robust}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:standing}}\DIFaddend )}
\DIFaddbegin \DIFadd{From a bent configuration, the robot has to stand up using a wall as support, and climbing a 25-cm high step.
}\DIFaddend 

\begin{figure}
  \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1\linewidth]{figures/stair}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1\linewidth]{figures/standing}
  \DIFaddendFL \caption{
           HRP-2 in the \DIFdelbeginFL \DIFdelFL{steep stair climbing }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{standing }\DIFaddendFL scenario. }
		   \DIFdelbeginFL %DIFDELCMD < \label{fig:stair_robust}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{fig:standing}
\DIFaddendFL \end{figure}


\DIFaddbegin \noindent\DIFadd{\textbf{Contacts involved:} All (both feet and hands).
}

\noindent\DIFadd{\textbf{Heuristics:} $h_w$ for the feet, $h_{\textrm{\it EFORT}}$  for the hands.
}
...
%DIF > ~ is moved twice, with the left foot in between, before the configuration allows HRP-2 to move its hand.
%DIF > ~ Because the contacts are tried in a FIFO manner, the fact that the output contact sequence is acyclic shows that a cyclic approach (with a finite state machine for instance) is not sufficient
%DIF > ~ for the computed path. The reason for this is not reachability, but equilibrium. The planning is slower than for the stair scenario (because the contact generation fails more),
%DIF > ~ though it remains compatible with \gls{interactive} performances. % \adnote{Maybe define what you mean by 'interactive' and 'real-time' at some point.}
...
%DIF > ~ \begin{figure}
  %DIF > ~ \centering
  %DIF > ~ \includegraphics[width=1\linewidth]{figures/stair}
  %DIF > ~ \caption{
           %DIF > ~ HRP-2 in the stair climbing scenario. }
		   %DIF > ~ \label{fig:stair_robust}
%DIF > ~ \end{figure}

\DIFaddend The goal is to climb three 15-cm high steps.
...
\DIFdelbegin \subsubsection{\DIFdel{HRP-2 -- Standing up (Figure~\ref{fig:standing})}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
\DIFdel{From a bent configuration, the robot has to stand up using a wall as support, and climbing a 25-cm high step.
}%DIFDELCMD < 

%DIFDELCMD < \begin{figure}
%DIFDELCMD <   \centering
%DIFDELCMD <   \includegraphics[width=1\linewidth]{figures/standing}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{HRP-2 in the standing scenario. }}
		   %DIFAUXCMD
%DIFDELCMD < \label{fig:standing}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < \noindent%%%
\DIFdel{\textbf{Contacts involved:} All (both feet and hands).
}%DIFDELCMD < 

%DIFDELCMD < \noindent%%%
\DIFdel{\textbf{Heuristics:} $h_w$ for the feet, $h_{\textrm{\it EFORT}}$  for the hands.
}%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdelend \subsubsection{HRP-2 -- Car egress (Figure~\ref{fig:car})}
...
%DIF > ~ \input{body/appendix_algo}
%DIF > ~ \input{body/appendix_hpp}

...
\as{Thanks for the comment. In fact, the github urls are not pointing to branches, but to release versions of the code, presented as zip files, which are in fact permanent
...
website (\url{http://stevetonneau.fr/files/publications/isrr15/tro_install.html}), and provided all the required information to install the material in it. The user has the choice to download the code either from the github permanent link or directly from the author's website directly.}

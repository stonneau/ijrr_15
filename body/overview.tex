...
\end{description}
C1 and C3 define \glslink{contact feasible}{\textit{contact feasibility}}, which is easier to verify than \glslink{equilibrium feasible}{\textit{equilibrium feasibility}} because it does not require the computation of contact forces.
...
However, even verifying \glslink{contact feasible}{\textit{contact feasibility}} is too computationally expensive because it requires the projection of the whole-body configuration into a contact posture.
...
We approximate \glslink{contact feasible}{\textit{contact feasibility}} with the \textit{reachability condition}, a geometrical criterion based on this intuitive description.
...
as well as extra DOFs not related to the limbs (for instance, HRP-2 has two extra DOFS in the torso, such that we have $r=8$). Abusively, for simplicity,
we refer to the root as the kinematic chain described by all these DOFS.
$R$ is composed of $l$ limbs $R^k, 1 \leq k \leq l$, attached to the root.
...
%\label{sec:A}
\medskip
The volume encompassing the root of the robot is denoted $W^0$.
...
$W^k(\mathbf{q}^{0})$ (for $1 \leq k \leq l$) as the volume $W^k$ translated and rotated according to the root configuration $\mathbf{q}^{0}$.

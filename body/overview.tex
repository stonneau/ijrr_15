...
We define $C_{Contact}^0$ as the set of \gls{contact feasible} root configurations (that can satisfy C1 and C2).
...
We approximate $C_{Contact}^0$ with a set $C_{reach}^0$, defined by the \textit{reachability condition}, a geometrical criterion based on this intuitive description.
In Section~\ref{rbprm} we detail how  $C_{reach}^0$ is sampled to compute a guide path with a dedicated planner, the Reachability-Based RRT---RB-RRT--- (Figure~\ref{fig:framework}---\Pa).

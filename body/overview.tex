...
\Pa and \Pb are addressed sequentially: we first plan a root guide path \deladp{with the \textit{reachability condition}}, before
...
\repadp{Similarly to}{As for} previous work~\citep{Bouyarmane2009} the path must be \textit{equilibrium feasible}: the root configurations \deladp{of the path} must be \repadp{extensible}{extended} into full-body static equilibrium configurations. Previous work verifies \textit{equilibrium feasibility} by explicitly computing such a \deladp{joint} configuration. 
...
More precisely, a root configuration is \textit{contact reachable} if the root scaled by a \addadp{user-defined} factor $s \geq 1$ is not in collision, while the reachable workspace is in collision.
...
The algorithm takes advantage of the fact that each root configuration is fixed to generate the contact by considering each limb individually. Candidate limb configurations resulting in the end-effector being close to the environment are selected from a database (\repadp{computed offline independently}{independent} from the environment).
...
\section{Notation \deladp{conventions} and definitions} \label{notations}

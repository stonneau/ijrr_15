...
\section{Results}
\label{sec:results}
In this \repadp{section}{Section} we present some of the results obtained with our planner. The complete sequences \deladp{computed} are shown in the companion video.
...
At the end of this \repadp{section}{Section}, we discuss the role of the parameters of our framework\deladp{ on the output}. We then provide the \textit{interactive} computation times obtained in each case.
We also compare the times obtained with HRP-2 with respect to previous works.

These scenarios \repadp{complement}{complete} the ones demonstrated on virtual avatars (Figure~\ref{fig:robots_old}) in our previous ISRR paper~\citep{tonneauisrr15} and video\footnote{\url{http://youtu.be/LmLAHgGQJGA}}.
...
a start and goal root configurations are provided as \deladp{an} input \deladp{of the scenario}.
...
The goal is to climb three 15-cm high steps. This height requires HRP-2 to use a ramp to perform the task. \adnote{It is not clear why, so either explain why, or remove the comment.}
...
From a bent configuration, \repadp{the robot has to stand up}{a standing-up motion is computed in a constraining environment. The resulting motion involves} using a wall as support, and climbing a 25-cm high step.
...
\repadp{In this scenario---inspired from the DRC car egress (\url{http://cpc.cx/edH})---HRP-2 has}{This scenario is inspired from the DARPA-Robotics-Challenge car egress scenario (\url{http://cpc.cx/edH}). HRP-2 has to find the contact sequence that allows it} to step out of the Polaris car.
...
The quadruped robot \repadp{must}{is given the task to} cross a rubble composed of bricks rotated at different angles and directions.

\begin{figure}
  \centering
  \includegraphics[width=1\linewidth]{figures/darpa}
  \caption{
           Robust crossing of rubbles by HyQ \deladp{($b_0 > 20$)}. }
		   \label{fig:darpa}
\end{figure}
...
In this long scene, HyQ \repadp{has to cross a 55-cm large hole, followed by}{is first required to cross a 55-cm large hole; then, to cross} a narrow ``bridge'', only 25-cm large.
...
\begin{figure}
  ...
  \caption{
           Crossing a hole contact sequence for HyQ \deladp{($b_0 > 4$)}. }
		   \label{fig:HyQ_obs}
\end{figure}
...
We discuss the several factors \adnote{Either use 'the' or use 'several'} that influence the outcome of our planner: the root scaling factor $s$ (Section~\ref{sec:scaling}), the \deladp{choice of} heuristics for
...
is computed empirically based on statistical analysis \adnote{Are you sure you wanna use the word 'statistical'?} or trials and errors.
...
We then computed the sensitivity of the reachability condition (\repadp{percentage}{proportion} of configurations in $C_{Reach}^0$, effectively belonging to \gls{$C_{Contact}^0$}).
Similarly we computed the specificity of the condition (\repadp{percentage}{proportion} of configurations \textbf{not} in $C_{Reach}^0$, effectively \textbf{not} belonging to \gls{$C_{Contact}^0$}).
...
As it can be expected, the scaling results in a high \repadp{increase}{decrease} of the sensitivity, with a decrease of the specificity.
...
\begin{tabular}{c | c | c}
 ...
 \hline
   ...
   1.5 & 99\% & 90.5\%\\
 \end{tabular}
\caption{Sensitivity and specificity values \addadp{of} the reachability condition, depending on the scaling value \addadp{$s$} of $W^0$.}
\label{tab:scale}
\quad
\end{table}

\subsubsection{\repadp{Choosing the heuristics}{On the choice of heuristics}:} \label{sec:heuristichoices}
In our conference paper~\citep{tonneauisrr15}, the computed motions were \deladp{only} generated using the EFORT heuristic.
EFORT is designed for tasks requiring to exert important forces (such as pushing / pulling / climbing). 
\repadp{In}{Regarding} locomotion tasks, such as the stair scenario, one issue with EFORT is that it tends to generate
configurations close to singularities (and joint limits). While this does not significantly impact
the generation of the plan, the resulting interpolation \deladp{on the real robot} turned out to be harder.
...
\adnote{Maybe you should say which discretization step you used.}
...
We measured the computation time spent in each \repadp{part}{aspect} of the algorithm, and analyzed their success rate.
...
Table~\ref{tab:requestime} summarizes the \deladp{performance measurements obtained, in terms of} computation times.

\repadp{F}{Considering the repartition of the computation time, f}or HRP-2, most of the time \repadp{was}{is} spent performing inverse kinematics.
...
thus allowing to consider \gls{interactive} applications\repadp{ and}{. This would allow to consider} online autonomous planning of the robot motion.


\subsubsection{Success rates:}
Table~\ref{tab:sucess_planning} summarizes the successful planning rates\deladp{ obtained for each scenario}.
Despite the complexity of the scenarios addressed, as well as the approximations made in our formulation, our planner \repadp{succeeded}{succeeds} in the large majority of cases.

Table~\ref{tab:requestpercent} presents\deladp{, for each scenario,} the rate of \addadp{successful} contact generation\deladp{ that succeeded}. Note that a failure in contact generation for a root configuration is not equivalent to a failure in the contact plan. It simply means that another limb was tested for contact generation for the same root configuration.
As expected, the more constrained scenario, the car egress \deladp{one}, provides the \repadp{worst}{less satisfying} results, despite the high success rate of the planner.
...
The most efficient (and immediate) approach to obtain a valid contact plan as fast as possible \repadp{would be}{is probably} to launch in parallel several instances of the planner (our current implementation is single-threaded) and to use any successful result as a plan for solver $\mathcal{P}_3$.

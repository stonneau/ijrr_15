...
At the end of this Section, we analyze the influence of the parameters of our framework on the output, and provide the \textit{interactive} computation times obtained in each case.
...
contact generation, 
%~ the desired value of the robustness parameter $b_0$, 
...
\deladp{From a pragmatic point of view, }regarding \deladp{the} computation times, \deladp{we claim that} our approach provides a satisfying compromise between completeness and efficiency.
Indeed, \deladp{the advantage of the framework is that} when the contact generation fails, it does so rapidly, which allows us to rapidly re-plan \deladp{a new contact sequence} with a reasonable chance of success.
The most efficient (and immediate) approach is probably to launch in parallel several instances of the planner \deladp{for a given problem} (our current implementation is single threaded) and to apply any successful result.

...
For all scenarios, the video displays a valid contact sequence outputs by our planner (i.e. $\mathcal{P}_2$ result).
They are then validated by exhibiting a dynamically-consistant solution to the interpolation problem $\mathcal{P}_3$, which is displayed at the video end.
Finally, a last example suggests possible applications to dexterous manipulation.
These scenarios complement our previous results with virtual avatars (Figure~\ref{fig:robots_old})~\citep{tonneauisrr15}.
%
%At the end of the video, we validate our contact plans by exhibiting a solution to the interpolation problem $\mathcal{P}_3$.

In this section, we first reports the scenarios that were solved by our planner (Sec.~\ref{sec:scenarios}) 
before discussing  the role of the parameters of our planner (Sec.~\ref{sec:influence}) and its performances (\textit{interactive} computation times, success rates -- Sec.~\ref{sec:perf}). 
%We then provide the \textit{interactive} computation times obtained in each case.
...
\centering
  \begin{overpic}[width=1\linewidth]{figures/robots_old}
		...
		%~ \put (68,27) {4.b)} 
	\end{overpic}
\caption{Virtual avatars in various scenarios demonstrated in our conference paper~\cite{tonneauisrr15} (see video on YouTube \url{http://youtu.be/LmLAHgGQJGA}).}
		   \label{fig:robots_old}
\end{figure}

\subsection{Description of the scenarios}\label{sec:scenarios}
...
is computed empirically based on use-case analysis or trials and errors (the same value is used in all the scenarios).
...
For HRP-2 and HyQ we decided to set $s^*=1.2$.
...
This parameter is similar to the step duration used e.g. in~\cite{Mordatch:2012:DCB:2185520.2185539}.
...
\begin{tabular}{ l | c}
 ...
 \hline
   ...
   Obstacle race        & 88\% 	\\
 \end{tabular}
\caption{ Rates of successful complete contact planning for each scenario.}
...
\begin{tabular}{ l | >{\centering\arraybackslash}m{65pt} | >{\centering\arraybackslash}m{35pt} | >{\centering\arraybackslash}m{35pt} | c}
  &  Success rate of the contact generator & Kinematic failure & Equilibrium failure \\
 \hline
 ...
 \end{tabular}
\caption{Success rate of the contact generator , whose failures may be due to kinematic issue or equilibrium issue. 
%% Success rates obtained for the generation of static equilibrium contact configurations for each scenario, rounded to the first decimal. Column 1 indicates 
%% indicates the rate of contact generation that suceeeded. In the cases where the generation fails, it can be
%% either a kinematic issue (column 2), or because no contact configuration led to a static equilibrium configuration (column 3). 
%Note that a failure in the contact generation is not equivalent to a failure of the contact planning algorithm.
}
...
Despite the complexity of the scenarios, our planner succeeded in the large majority of cases.

It is also interesting to observe the causes of failure of the contact generator (Table~\ref{tab:requestpercent}).
As it iterates until sucess, one failure of the contact generator is not equivalent to a failure in the contact plan (but corresponds to increased computation time).
...
These results correspond to an empirical validation of the \textit{reachibility condition}.
As previously explained, this condition is not sufficient to guarantee that a path $\mathcal{P}_1$ is expendable in a sequence~$\mathcal{P}_2$.
However we see in practice that, even in complex scenario, this condition is a fair approximation.
Moreover, when the contact planning fails, it fails rapidly. 
It is then immediate to simply invalidate the corresponding edge of the planning graph and to rapidly re-plan it with a reasonable chance of success.

%% These results confirm that our approach provides a satisfying trade-off between completeness and efficiency, thus allowing to consider online planning
...
\subsection{Validation of the contact plans}
The contact sequences computed by our planner are finally used to generate a whole dynamics trajectory using~$\mathcal{P}_3$.
For this last stage, we relied on existing formulation, either directly using existing solvers~\cite{Carpentier2016} or implementing our own version a $\mathcal{P}_3$ solver, described in Appendix~\ref{app:optim}.
Because $\mathcal{P}_3$ is not a contribution of this paper, we do not provide here computation scores (see~\cite{Carpentier2016} for details).
The resulting whole trajectories are dynamically consistant: they are smooth; contact forces corresponds to the robot motion; joint motion and torques are within the acceptable ranges specified by the robot manufacturers.
All the resulting movements have been executed in a complete dynamic simulator.
We executed one of them using the real HRP-2 robot
%
...
%
Furthermore, $\mathcal{P}_3$ remains an open problem in the presence of obstacles. 
The only valid scenarios completely addressed  in previous works are thus the stair-climbing scenarios of different heights  in \cite{Hauser06usingmotion}, and the table-egress scenario in~\cite{DBLP:conf/iser/EscandeKMG08}(whose  complexity is similar to our car-egress scenario).
Both our scenarios are tested with HRP-2.

Table~\ref{tab:compprev} presents the computation times for these scenarios.
The scores clearly demonstrate that our approach is order of magnitude faster than previous works.

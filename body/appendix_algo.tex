...
in computing a balance configuration with only one contact switch occuring.
If $g$ failed in achieving this, the method \textsc{RepositionContacts} is called.
It repositions one end effector (either a free limb, or the oldest active contact) towards a new balanced contact position if possible.
...
\caption{Full body contact generation method} \label{interpolate}
	...
	\Function{$g$}{$pState$,$\mathbf{q}^0$}
		...
		\For {\textbf{each} Limb $k$ in $previous.freeLimbs$}
			\State /*Generate a balanced contact*/
...
\textsc{GenerateContact}$(\mathbf{q}^0,k)$ is a call to the contact generator presented in Section~\ref{sec:single_contact}. It generates a balanced contact configuration, and assigns the corresponding configuration to $k$.

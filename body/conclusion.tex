...
As demonstrated in the results section, the main interest of our method is reduced algorithmic complexity, leading to interactive implementation. This property is critical for 
online applications with the robot and was not reached by any of the previous contributions. Our method addresses highly constrained environments while improving the search time by orders of magnitude. This high performance is reached at the cost of some approximations that we discuss here. 
%~ In previous approaches, we identified 2 sources of high computational cost. 

The first approximation occurs when verifying the \contactreachability\ ($\mathbf{q}^0 \in C_{Contact}^0$).  Our \textit{reachability condition} ($\mathbf{q}^0 \in C_{Reach}^0$) is computationally efficient and provides an accurate approximation of $C_{Contact}^0$ (Section~\ref{sec:scaling}). This is demonstrated by the second column of Table~\ref{tab:requestpercent}, and illustrated by Figure~\ref{fig:dedefeas}. Indeed, in the large majority of cases, (84\% in the worst car egress case), we are able to find a contact configuration for any configuration in $C_{Reach}^0$.
...
Moreover, refining the reachibility condition to makes it more accurate while keeping the separated approach seems a fruitful research direction.

\subsection{Addressing $\mathcal{P}_3$}
This paper focuses on contact planning of multi-contact locomotion. Following the spirit of previous works~\cite{Bretl:2006:MPM:1124573.1124585,DBLP:conf/iser/EscandeKMG08,7140082},
it does not address directly the continuous motion generation. Problem $\mathcal{P}_3$ remains an active research topic~\cite{Carpentier2016, herzog2015trajectory},
...
Our results demonstrate that our method allows a pragmatic trade-off between three 

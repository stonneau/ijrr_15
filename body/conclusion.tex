...
As demonstrated in the results section, the main interest of our method is time performance. Time performance is critical \deladp{to consider using a multi-contact planner} for 
...
The first approximation is the verification of \contactreachability\ ($\mathbf{q}^0 \in C_{Contact}^0$).  Our \textit{reachability condition} ($\mathbf{q}^0 \in C_{Reach}^0$) is computationally efficient and provides an accurate approximation of $C_{Contact}^0$ (Section~\ref{sec:scaling}). This is demonstrated by the second column of Table~\ref{tab:requestpercent}, and illustrated by Figure~\ref{fig:dedefeas}. Indeed, in the large majority of cases, (84\% in the worst case \adnote{I do not get where this 84 comes from}), we are able to find a contact configuration for any configuration in $C_{Reach}^0$.
...
This paper focuses on \deladp{the} contact planning \deladp{aspect} of multi-contact locomotion. While it does not address directly the continuous motion generation,
we had to prove that the plans produced by our planner \deladp{are useful and} can be executed by the considered robots. Problem $\mathcal{P}_3$ remains \repadp{an active research topic~\cite{Carpentier2016, herzog2015trajectory}}{open},
...
Our contribution to \Pb is a fast contact generation scheme that can \repadp{optimize user-defined criteria}{take into
...
%
\textbf{Regarding performance}, our framework is \deladp{really} efficient in addressing both \Pa and $\mathcal{P}_2$. This results in \gls{interactive} computation times.
%
...
Furthermore, because of this approximation, the guide search is not complete. The choice is deliberate, because we \repadp{believe}{are convinced}
...
Solving $\mathcal{P}_3$ requires addressing efficiently the collision avoidance problem \deladp{during the interpolation},	but also improving

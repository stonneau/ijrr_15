...
The first problem is \Pa, that consists in computing a guide path for the root of the robot\deleted{ that can be extended}; the second problem is \Pb, that is the computation of a discrete sequence of contacts along the root path.
...
of the volumes $W^0$.
%
\textbf{Regarding performance}, our framework outperforms existing methods in addressing either \Pa or \Pb, \adnote{saying you outperform existing methods when you do not show any comparison is risky, the reviewers might ask for comparisons} leading to computation costs close to real-time in
\replaced{known static}{statically known} environments.
%
...
The low computational burden of our framework however allows for fast replanning \replaced{in case of failure}{when the solution fails}.
...
We have already been able to achieve this for HRP-2 for some of the computed sequences~\citep{Carpentier2016}, but we \replaced{admit}{concede} that additional
...
\adnote{The 'future work' part seems a bit poor to me}

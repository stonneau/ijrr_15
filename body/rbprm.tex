...
For a contact to be possible, an obstacle $O_i \in O$ necessarily intersects the reachable workspace $W(\mathbf{q}^{0})$ of the robot (Figure~\ref{fig:contact_gen}--1). Also the torso of the robot $W^0(\mathbf{q}^{0})$ must necessarily be collision-free.
...
\textbf{sufficient condition\addadp{s}:}
...
\subsubsection*{}
This Section has presented a guide path planner for the geometric root of a robot. This planner is implemented as a low-dimensional sampling-based 

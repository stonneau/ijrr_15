...
We consider the issue of computing a normalized guide path $\mathbf{q}^0(t) : [0,1] \longrightarrow SE(3)$ for the geometrical \adnote{why the root is geometrical? Isn't the path to be geometrical?} root of a multiped robot, connecting \adnote{user-defined} start and goal configurations. As said in the previous section, the goal is to enforce that any configuration $\mathbf{q}^0$ of the guide is \replaced{geometrically}{truly} feasible, i.e. can be mapped to a configuration in contact. We denote by $C_{contact} \subset SE(3) \times \mathbb{R}^n$ the contact submanifold of the robot\added{, that is the set of configurations in which the robot is in contact with the environment}.
...
We say that a root placement $\mathbf{q}^{0}$ is \textit{\replaced[remark={I don't replace 'truly feasible' with 'geometrically feasible' from now on because I don't know whether you like it}]{geometrically}{truly} feasible} if there exists at least one bijective mapping $\pi$
such that
\begin{equation}
...
Thanks to this small set of modifications, the high dimensional truly feasible path planning problem is reduced to a standard geometry and collision checking problem, in a much lower dimension (6 for Hyq, 8 for HRP-2 that has \replaced{joints in the}{a flexible} torso). \replaced{By doing do}{In so doing} we make the problem tractable for sampling-based approaches, with extremely efficient computation times.

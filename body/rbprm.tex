...
We say that a root placement $\mathbf{q}^{0}$ is geometrically feasible if there exists at least one bijective mapping $\pi$
such that
\begin{equation}
...
The set of all geometrically feasible root placements is denoted by $C_{reach}$. By extension, a path $\mathbf{q}^0(t)$ is truly feasible if $\forall t \in [0,1], \mathbf{q}^0(t) \in C_{reach}$.

For a two-stage acyclic contact planner to be exact and complete, we need the combination of two conditions on a guide trajectory generator: all the generated trajectories must be \textit{geometrically feasible} (sufficient condition); the guide planner must be complete, i.e. it must be able to discover any truly feasible trajectory (necessary condition).
...
\subsubsection*{Geometric feasibility: necessary condition}

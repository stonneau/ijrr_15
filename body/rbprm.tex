...
\section{Root path planning in the contact reachable space ($\mathcal{P}_1$)}
\label{rbprm}
...
Eq.~\eqref{eq:reachc} thus defines the \textit{reachability condition}. We fix a value $s^*$ for $s$ and define  $C_{Reach}^0 = C^0_{s^*}$.
...
$C_{Reach}^0$ can be sampled efficiently thanks to \eqref{eq:reachc}, and can thus be used with a standard motion planner.
The only significant change is to replace the collision checking with the \textit{reachability condition}.
Our current implementation of these modifications is based on the Bi-RRT planner \citep{770022} provided by the HPP software~\citep{7759083}. \\
...
%algorithm. Given start and goal configurations, it outputs a continuous path for the root of the robot. 

The result of this section is a guide path for the root of the robot (problem $\mathcal{P}_1$), computed with simple modification over a bi-RRT planner using the reachability condition.
We now explain how this path of the root can be extended to a sequence of whole-body contact configurations.

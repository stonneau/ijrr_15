...
\begin{itemize}
\item The guide trajectory must satisfy a property guaranteeing the existence of a contact sequence to actuate it\endnote{This property is related to the controllability of the root actuated by the contact forces, but for discrete bounded properties.}. We call this property \textit{true feasibility}. This property has not been studied yet: the only way to validate a waypoint in the trajectory is to explicitly compute the contacts, which is computationally not reasonable \citep{Bouyarmane2009}.
...
In particular, we detail the robust balance criteria designed to ensure the equilibrium of the robot. Using HRP-2 as a running example, we describe how $C_{reach}$ is approximated to address
\Pa, and provide the pseudo code of the contact generation algorithm implemented to find balanced contact sequences.

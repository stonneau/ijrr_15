...
The true feasibility of the path provides guarantees that the contacts can be created, and reduce the dimensionality of the problem (at each step the root position is fixed). The theoretical completeness of the approach is partially traded for efficiency: instead of sampling on-line
limb configurations and projecting them onto contact, we precompute off-line a large database of limb configurations, stored in a spatial structure optimized

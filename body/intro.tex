...
Compared to previous approaches of the litterature, our solution has two main novelties: 
...
\textbf{The reachability condition is the key to the strict separation between $\mathcal{P}_1$ and $\mathcal{P}_2$}, hence to the low complexity of our planner.
However, the reduction may result in failures. We demonstrate empirically its interest, both in the high success rate and in the low computation times, by a deep experimental validation, with two realistic robot models (humanoid and quadruped) in various scenarios, involving statistics of the relevance of our hypotheses.
These two properties allow us to plan, and re-plan upon failure, multi-contact sequences at \gls{interactive}$^1$ rates.
...
\subsection{Organization and comparison with preliminary paper~\cite{tonneauisrr15}}
The present paper is an extension of our ISRR conference paper~\cite{tonneauisrr15}, that it aims at replacing.
The core methodological part corresponds to the same motion planner, with important complements listed below.
The present paper also proposes a novel experimental section, with statistical validation of the planner hypothesis using realistic robot models.
 
After presenting an overview of our method (Sec.~\ref{overview}) and introducing our notations (Sec.~\ref{notations}),
we present our solution to address $\mathcal{P}_1$ (Sec.~\ref{rbprm}) and $\mathcal{P}_2$ (Sec.~\ref{sec:contact}) , which corresponds to the same motion planner as the one presented at \cite{tonneauisrr15}. The two sections hence are reformulation of the preliminary paper. 
Three important novelties have been added to the planner: the pseudo-code of the algorithm (Sec.~\ref{app:contact}), a novel criterion for
static equilibrium (Sec.~\ref{sec:Equil}), and the release of the source code of the planner (Sec.~\ref{app:hpp}).

The other novelty of the paper is a rigorous experimental validation of the approach on actual robot models (Sec.~\ref{sec:results}).
We had to introduce a complete framework for multi-contact motion synthesis, in order to validate our contact plans. It comprises an interpolation method to solve the problem $\mathcal{P}_3$, based on a reformulation of our previous work~\cite{Carpentier2016}. The generated motions are verified to be physically consistent, using our implementation of a state-of-the-art simulation algorithm~\cite{Kaufman2008}. 
These aspects of the framework are presented in details in the paper, but are not novel \textit{per se}. The novelty lies in the complete validation of the contact planner with real robot models, for which the planning is much harder with respect to the avatars used in \cite{tonneauisrr15}, because of stricter kinematic constraints.
...
%Finally, as opposed to our previous paper, we demonstrate the algorithms with actual robotic models. These models are more constrained kinematically than the virtual avatars used before, thus making the planning problem harder. Through an extensive empirical analysis, we demonstrate that the method remains efficient and applicable in this context.

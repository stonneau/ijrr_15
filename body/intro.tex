...
After years of research, such robots can autonomously walk on flat ground, but struggle to navigate in more complex environments, e.g. to climb stairs using a handrail.
In such a case a humanoid robot must plan the contacts it musts create with its feet and hands. Computing such a movement is an open motion-planning problem called acyclic contact planning.
...
To that end, we introduce a reduction property that we name the reachability condition.
This condition relaxes the strict contact condition.
It induces a low-dimensional space, called the \textit{contact reachable} space, that has a Cartesian topology, i.e. it can be efficiently sampled.
...
This approach of the problem enables us to separate the contact planning problem in two tractable subproblems.

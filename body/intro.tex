...
Planning the locomotion of a multipedal robot in a complex environment - made of stairs, constraining obstacles or uneven surfaces - is still an open issue. Most multipedal locomotion systems focus on cyclic walking gaits \citep{Kajita03a}, but executing this behavior on constraining environments is dangerous if not impossible. In such a context, the robot has to perform a particular class of motion planning called acyclic contact planning, in which the robot must be in contact with the environment at every configuration to maintain the static equilibrium. 

The lack of solution to this planning problem has been highlighted by Atkeson \citeauthor{atkensondarpa}. Actually, the current approach to locomotion planning tends to avoid obstacles as much as possible, instead of using them by making contacts to facilitate motion. One main reason is that contact planning is algorithmically difficult, both because of the proximity to obstacles which makes tedious the sampling of collision-free configurations, and because of the foliation of the configuration space where zero-measure manifolds intersect in a combinatorial manner \citep{simeon-manipulation-04}.

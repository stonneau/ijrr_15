...
\label{sec:contact}
As an input of this stage, we consider a truly feasible guide path $\mathbf{q}^0(t) : [0,1] \longrightarrow SE(3)$ for the root of the robot $R$. We now consider the second problem of computing a discrete sequence of contact configurations $\mathbf{Q}^{\overline{0}}$ for the limbs of the robot along $\mathbf{q}^0(t)$. \deleted[remark={the input is not truly feasible, actually it is not even geometrically feasible, it is only approximately geometrically feasible}]{From true feasibility we know that such a trajectory exists.}
...
and root location.
Then, on this method we build an iterative algorithm to generate a discrete sequence of contact configurations in equilibrium\deleted{, along a truly feasible path}.
...
We consider a limb $R^k$  and a root configuration $\mathbf{q}^0$\deleted{ truly feasible regarding $R^k$}. FIGURE
From true feasibility we know that there exists an infinite list of mappings $P = [\pi_0,..,\pi_i, ...]$ that verify \eqref{eq:pi} for $\mathbf{q}^0$.
...
\begin{enumerate}
\item Generate randomly a \replaced{collision-free}{valid} limb configuration;
...
\item Figure~\ref{fig:contact_gen}--2 and 3--:\adnote{This notation for subfigures is not clear, consider using subfigures, e.g. 2a, 2b} At runtime, when contact creation is required, retrieve from the octree the list of samples $S \subset C_{Contact}^{\epsilon}$ close to contact;
...
\item If a valid \adnote{being valid depends also on the other contacts because it regards also equilibrium, doesn't it? Then this assessment depends on the other contacts previously generated. Doesn't this have implications on the completeness? Even if it doesn't, isn't it worth to talk about it?} solution is found, stop. Otherwise remove the element from $S$ and go back step 5.
\end{enumerate}

This approach has two main advantages.
First, this allows us to select a large number of candidates in a single proximity request.
Having several candidates is interesting, because it allows to compare them \replaced{using}{regarding} a user-selected heuristic $h$, thus obtaining
...
We thus have defined a real-time\deleted{, extremely efficient} contact generator that can be as complete as required by the context.
...
the previous contacts while allowing for static equilibrium.
...
Fig.~\ref{fig:break_contact} illustrates the contact-persistence strategy.
...
\begin{enumerate}
\item Only one contact creation can happen between two consecutive states. This increases the odds that the interpolation between the two states \deleted[remark={not 100\% sure}]{will} be feasible; \adnote{Before you said that you ensured that transition between two states is feasible...}

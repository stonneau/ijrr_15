...
A contact generator is defined for a given limb $R^k$, and only affects the dofs of $R^k$.
Given a robot configuration, it computes a configuration $\mathbf{q}^k$ such that $R^k$ is in contact and the robot $R$ is in static equilibrium:
\begin{equation}
...
Contact generation is typically adressed by randomly sampling a limb configuration, before projecting the considered effector onto the closest surface with an inverse kinematics solver.
...
between an effector and an obstacle is lesser than $\epsilon \in \mathbb{R}$.
...
of the $N$ parameter \citep{Tonneau2014}. Our criterion to assert efficiently the static equilibrium of the robot, as well as the heuristics $h$ used are detailed in Appendix~\ref{sec:heuristics}.
...
\begin{enumerate}
\item At most one contact creation happens between two consecutive steps. 
...
The set of rules defined in this Section define a deterministic contact planning algorithm.

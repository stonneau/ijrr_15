...
In previous contributions\adnote{Add reference here}, a contact plan is defined as a sequence of \deladp{configurations that describe a} quasi-static equilibrium configurations
for each \repadp{contact}{existing} phase \deladp{of the motion}. For instance, a walk cycle would be described by three key configurations: a double-support configuration, a single-support configuration (a contact is broken), and another double-support configuration (a contact is created). 
...
This representation is sufficient to describe all the contact constraints of the problem. Furthermore it removes the need to have single-support quasi-static configurations as in the example. 

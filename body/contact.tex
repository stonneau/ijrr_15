...
$\mathbf{q}^0(t) : [0,1] \longrightarrow$ \gls{$C_{Reach}^0 \simeq C_{Equil}^0$}. This contact planner is built on top of
...
\subsection{\repadp{Contact}{Definition of a contact} generator}
...
\repadp{The contact generation treats each limb $R^k$ independently.}{A contact generator is defined for a given limb $R^k$, and only affects the DoFs of $R^k$.}
Given a \repadp{configuration of the root and the other limbs}{robot configuration}, it computes a configuration $\mathbf{q}^k$ such that $R^k$ is in contact and the robot $R$ is in static equilibrium:
\begin{equation}
...
Contact generation is typically adressed by randomly sampling a limb configuration, before projecting the \deladp{considered} effector onto the closest surface with an inverse kinematics solver.
...
of the \repadp{parameter $N$}{$N$ parameter}~\citep{Tonneau2014}. Our criterion to assert efficiently the static equilibrium of the robot, as well as the heuristics $h$ \deladp{used} are detailed in Appendix~\ref{sec:heuristics}.
...
\item \addadp{At most one contact is added between two consecutive configurations.} 
...
If two or more contacts \repadp{cannot be maintained between two consecutive configurations}{are broken in a single step}, one or more intermediate \repadp{configurations}{steps} are added.
...
Contacts are created using \deladp{three rules:} \addadp{a FIFO approach: we try first to create a contact with the limb that has been contact-free the longest. If the contact creation was not successful for a limb, the limb is pushed on top of the queue, and will only be tried again after the others.}
\begin{enumerate}
\item \deladp{At most one contact creation happens between two consecutive steps. }
%~ This increases the odds that the interpolation between the two steps be feasible;
...
\repadp{This set of rules}{The set of rules defined in this Section} defines a deterministic contact planning algorithm.

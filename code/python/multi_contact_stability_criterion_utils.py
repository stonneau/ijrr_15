...
#    b = A_red[:,1];
    ...
    for iiii in range(MAX_ITER):
        ...
        if(abs(a[i_DDalpha_min])>EPS):
            ...
            delta = sqrt(Dalpha_t**2 + 2*d[i_DDalpha_min]*(alpha_t-alpha_max))
            ...
            if(t<0.0):
                ...
                print "WARNING: Time is less than zero:", t, alpha_t, Dalpha_t, d[i_DDalpha_min], alpha_max; 
                t = (- Dalpha + delta) / d[i_DDalpha_min];
                if(t<0.0):
                    # If also the largest time is negative print an ERROR and return
                    print "ERROR: Time is still less than zero:", t, alpha_t, Dalpha_t, d[i_DDalpha_min], alpha_max; 
                    return (False, c0+alpha_t*v, Dalpha_t*v);

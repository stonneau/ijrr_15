...
\as{Thanks for the comment. In summary, we had to implement our simulator because we were not satisfied with commercial simulators. Our simulator implements the paper ``Staggered projections for frictional contact in multibody systems'', by Kauffman, Sueda, James and Pai, which corresponds to a state-of-the-art solution to the problem.
We considered other simulators, namely Mujoco, ODE, Gazebo, Bullet and Open-HRP. We discarded Mujoco because we work on Linux and it only provides a windows implementation. In our tests, we were not able to obtain coherent results with ODE and Gazebo, even for simple pendulum models. At the time of choosing which simulator to use, Bullet did not allow to express the dynamics
equations using joint coordinates, but only cartesian coordinates, which made the formulation much more complex. Finally, Open-HRP is a great tool, but it only provides an interface for position-controlled models, and not torque-controlled. For these reasons, and other historical ones, we implemented our own simulator. We understand that because we use an in-house implementation, it is harder to trust the results we obtain. However, the implementation is from a state of the art LCP solver, used by many in the community. In this new submission, we also provide the source of the simulator. As for the paper itself and the rest of the code, we put a considerable effort in providing the reader with all the elements that will lead to an objective opinion of the paper.}

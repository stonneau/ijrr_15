...
First of all, we would like to thank all the reviewer and the associate editor (AE).
We received three detailed and technical reviews that helped us to improve our paper.
In general, we agreed we most of the reviews and take them into account in the revised version of the paper.
The two main points that were raised by all reviewers was about i) the contributions of the paper with respect to our previous papers [20] and in a lesser extend [1] and ii) the organization of the paper, with some details postponed to appendix and some other deferred to cited papers.

\textbf{About the contribution of this paper with respect to our previous papers [20] and [1]}.
This paper is an extension of [20] (presented in the conference ISRR), that it aims at replacing.
...
Specifically, the extension with respect to [20] consist in:
\begin{itemize}
...
\textbf{About the organization of the paper and the use of appendices and citation to reduce the core of the paper}. In order to try to make the paper more readable, the first version was mixing three styles: details given in the core of the paper; details given in the appendices; details deferred in cited papers. While all reviewers agreed that using all three styles together was a bad idea, we received contradictory reviews about how to modify the paper. Following the recommandation of the AE, we have worked to make the paper self-contained with all the contributions in the core of the paper. 
...
In the following, we list all the questions and remarks of the reviewers, answer to direct questions and point the modifications that were brought to the paper.
